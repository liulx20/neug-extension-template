...
class IC12 {
 public:
  ...
  static execution::Context exec(const function::CallFuncInputBase& input,
    ...
    const std::string tag_class_name = args.tag_class_name;
    ...
    const label_t is_subclass_of_label =
        schema.get_edge_label_id("ISSUBCLASSOF");
    ...
    if (!first_name_col || !last_name_col || !tag_name_col) {
      THROW_RUNTIME_ERROR(
          "ic12: failed to load required LDBC property columns");
    }
    ...
    ldbc::foreach_knows_neighbor(
        graph, person_label, knows_label, root, [&](vid_t friend_vid) {
          ...
          if (pq.size() < kTopN) {
            const int64_t friend_id = person_id_col->get_view(friend_vid);
    ...
    ListColumnBuilder tag_names_builder(DataType(DataTypeId::kVarchar));
    ...
    for (size_t i = results.size(); i > 0; --i) {
      ...
      for (vid_t tag_vid : distinct_tags) {
        tag_values.emplace_back(
            Value::STRING(std::string(tag_name_col->get_view(tag_vid))));
      }
      tag_names_builder.push_back_elem(
          Value::LIST(DataType(DataTypeId::kVarchar), std::move(tag_values)));
