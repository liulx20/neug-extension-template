...
class IC14 {
 public:
  ...
  static void dfs_paths(const CsrView& out_view, const CsrView& in_view,
                        vid_t src, vid_t dst,
    ...
    for (auto it = out_edges.begin(); it != out_edges.end(); ++it) {
      ...
      if (on_path[v] && dist_from_src[v] == dist_from_src[src] + 1) {
        dfs_paths(out_view, in_view, v, dst, dist_from_src, on_path, path,
                  paths);
      }
    }
  ...
  static std::vector<int> score_paths(
      ...
      label_t post_label, label_t comment_label, label_t has_creator_label,
      label_t reply_of_label, const std::vector<std::vector<vid_t>>& paths) {
    ...
    while (!person_pairs.empty()) {
      ...
      if (nbr_set.size() == 1) {
        ...
        score_cache[pair_key(root, other)] +=
            scorer.pair_score_one(root, other);
  ...
  static execution::Context exec(const function::CallFuncInputBase& input,
    ...
    for (size_t idx : order) {
      ...
      for (vid_t v : paths[idx]) {
        ids.emplace_back(Value::INT64(person_id_col->get_view(v)));
      }
