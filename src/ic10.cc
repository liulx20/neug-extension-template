...
class IC10 {
 public:
  ...
  static int score_friend_posts(const StorageReadInterface& graph,
                                vid_t friend_vid,
  ...
  static execution::Context exec(const function::CallFuncInputBase& input,
    ...
    std::vector<bool> has_interests(graph.GetVertexSet(tag_label).size(),
                                    false);
    ...
    const size_t step_1_size =
        std::min(friends.size(), static_cast<size_t>(10));
    while (friends_index < step_1_size) {
      ...
      pq.push(PersonResult{score, v, person_id_col->get_view(v)});
    }
    ...
    while (friends_index < friends.size()) {
      ...
      if (pq.size() < kTopN) {
        ...
        const int64_t person_id = person_id_col->get_view(v);
