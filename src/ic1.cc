...
// Element type for universities/companies: STRUCT(orgName, orgYear,
// orgPlaceName)
 ...
 public:
  ...
  static execution::Context exec(const function::CallFuncInputBase& input,
    ...
    for (size_t i = results.size(); i > 0; --i) {
      ...
      universities_builder.push_back_elem(
          Value::LIST(kOrgPlaceTupleType, std::move(university_values)));
      ...
      for (auto it = work_edges.begin(); it != work_edges.end(); ++it) {
        ...
        tuple_vals.emplace_back(Value::STRING(std::move(country_name)));
