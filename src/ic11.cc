...
class IC11 {
 public:
  ...
  static execution::Context exec(const function::CallFuncInputBase& input,
    ...
    const std::string country_name = args.country_name;
    ...
    const auto& work_from_accessor = graph.GetEdgeDataAccessor(
    ...
    for (auto oit = orgs.begin(); oit != orgs.end(); ++oit) {
      ...
      for (auto wit = workers.begin(); wit != workers.end(); ++wit) {
        ...
        const int32_t work_from =
            work_from_accessor.get_typed_data<int32_t>(wit);
        ...
        const int64_t person_id = person_id_col->get_view(person_vid);
