...
class IC5 {
 public:
  ...
  static execution::Context exec(const function::CallFuncInputBase& input,
    ...
    std::priority_queue<ForumInfo, std::vector<ForumInfo>, ForumInfoComparer>
        que(cmp);
    for (const auto& pair : post_count) {
      ...
      if (que.size() < kTopN) {
        que.emplace(forum_vid, count, forum_id_col->get_view(forum_vid));
        continue;
      }
      ...
      if (top.post_count < count) {
        ...
        const int64_t forum_id = forum_id_col->get_view(forum_vid);
    ...
    if (que.size() < kTopN) {
      for (vid_t forum_vid : forum_list) {
        ...
        accessed_forum_set[forum_vid] = false;
        if (que.size() < kTopN) {
          que.emplace(forum_vid, 0, forum_id_col->get_view(forum_vid));
          continue;
        }
        const auto& top = que.top();
        const int64_t forum_id = forum_id_col->get_view(forum_vid);
